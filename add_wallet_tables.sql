-- Add wallet tables to the database
...
CREATE TABLE IF NOT EXISTS "WalletSettings" (
    ...
    CONSTRAINT "WalletSettings_userId_key" UNIQUE ("userId")
);
...
-- Add wallet relations to User table (if not already added)
-- Note: These relations are already defined in the Prisma schema
-- but we need to ensure the foreign key constraints are properly set up

-- Add wallet relations to existing User table
ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "wallets" INTEGER[];
ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "walletTransfersFrom" INTEGER[];
ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "walletTransfersTo" INTEGER[];
ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "walletSettings" INTEGER[];
ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "walletUserAccount" INTEGER[];

-- Note: The actual foreign key relationships are handled by Prisma
-- This SQL file is for reference and manual database setup if needed

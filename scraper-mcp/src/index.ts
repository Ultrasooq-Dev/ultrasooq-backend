...
async function callBackend(path: string, method: string = 'GET', body?: any): Promise<any> {
...
server.tool(
  'scrape_start',
  'Start a scraping job for a specific platform, category, and region',
  {
...
server.tool(
  'scrape_progress',
  'Get real-time scraping progress stats across all platforms or a specific job',
  {
...
server.tool(
  'scrape_pause',
  'Pause a scraping job or all jobs for a platform',
  {
...
server.tool(
  'scrape_resume',
  'Resume a paused scraping job or platform',
  {
    target: z.string().describe('Job ID number, platform name, or "all"'),
  },
  async ({ target }) => {
...
server.tool(
  'scrape_fix',
  'Fix a blocked or failed scraping job with a recovery strategy',
  {
...
server.tool(
  'scrape_queue',
  'View and manage the scraping job queue',
  {
...
server.tool(
  'translate_text',
  'Translate a batch of scraped product texts from Chinese to English via AI',
  {
...
server.tool(
  'translate_image',
  'Extract and translate text from a product image using Opus 4.6 vision',
  {
...
server.tool(
  'category_import',
  'Fetch and import the category tree from a source platform',
  {
...
server.tool(
  'export_batch',
  'Export scraped products to JSON files for backup and audit',
  {
...
server.tool(
  'import_to_db',
  'Bulk import an exported batch into the Ultrasooq database with INACTIVE status',
  {
...
server.tool(
  'monitor_health',
  'Get full health report: error rates, block status, queue depths, ETAs, platform progress',
  {},
  async () => {
...
server.tool(
  'workflow_start',
  'Start the full 10M product mega scrape workflow — crash-proof, auto-resuming',
  {},
  async () => {
...
server.tool(
  'workflow_status',
  'Get the current workflow state, progress checkpoint, and full health report',
  {},
  async () => {
...
server.tool(
  'workflow_reset',
  'Reset the workflow state completely — use with caution',
  {},
  async () => {
